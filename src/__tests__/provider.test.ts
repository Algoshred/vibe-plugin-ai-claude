...
describe('ClaudeProvider', () => {
  ...
  describe('configureSession', () => {
    it('updates session config', async () => {
      const session = await provider.createSession(sessionConfig);
      await provider.configureSession(session.id, { model: 'claude-opus-4-20250514' });
  ...
  describe('sendPrompt', () => {
    ...
    it('throws for non-existent session', async () => {
      await expect(provider.sendPrompt('ghost', 'Hello')).rejects.toThrow('not found');
    });
    ...
    it('throws for terminated session', async () => {
      ...
      await expect(provider.sendPrompt(session.id, 'Hello')).rejects.toThrow('terminated');
    });
  });
  ...
  describe('cancelRequest', () => {
    it('throws for unknown session', async () => {
      await expect(provider.cancelRequest!('missing')).rejects.toThrow('not found');
    });
  });
  ...
  describe('attachFiles', () => {
    it('attaches files to an existing session', async () => {
      ...
      await provider.attachFiles!(session.id, [
        { filename: 'test.txt', mimeType: 'text/plain', content: 'hello', size: 5 },
      ]);
